...
def np_v2coory(v, coorH=512):
    return (0.5 - v / PI) * coorH - 0.5

def pano_connect_points(p1, p2, z=-50, w=1024, h=512):
    if p1[0] == p2[0]:
        return np.array([p1, p2], np.float32)

    p = np.array([[p1[0], p1[1]], [p2[0], p2[1]]])
    xy = np_coor2xy(p, z, w, h, 1, 1)
    ...
    vx = x2 - x1
    vy = y2 - y1
    us = np_coorx2u(coorxs, w) - 0.5*PI   #
    ps = (np.tan(us) * x1 - y1) / (vy - np.tan(us) * vx)
    cs = np.sqrt((x1 + ps * vx) ** 2 + (y1 + ps * vy) ** 2)
    vs = np.arctan2(z, cs)
    coorys = np_v2coory(vs, h)

    return np.stack([coorxs, coorys], axis=-1)

def calc_Rectangle(pa, pb, po):
    # AO is vertical to BO
    assert ((po[0]==pa[0])|(po[0]==pb[0]))
    if po[0] == pa[0]:
        ps = np.stack([po, pa, pb])
    else:
        ps = np.stack([po, pb, pa])

    xy = np_coor2xy(ps, 50, coorW=1024, coorH=512, floorW=1, floorH=1)
    if abs(xy[0, 0]) > abs(xy[0, 1]):
        ratio = xy[1, 0] / xy[0, 0]
    else:
        ratio = xy[1, 1] / xy[0, 1]
    pd = np_xy2coor(ratio * xy[None, 2, :], 50, coorW=1024, coorH=512, floorW=1, floorH=1)

    # return plt, prt, plb, prb
    p_res = np.concatenate([ps[[0, 2, 1], :], pd], axis=0)
    if p_res[2, 1]<p_res[0, 1]:
        p_res = p_res[[2, 3, 0, 1], :]
    [st, ed] = p_res[:2, 0]
    if (st > ed) & ((st - ed) < 512):
        p_res = p_res[[1, 0, 3, 2], :]
...
def getPanoRectangle(pa, pb, po):
    ps = calc_Rectangle(pa, pb, po)

    z = 100*(ps[0, 1] < 256) - 50
    line1 = pano_connect_points(ps[0, :], ps[1, :], z, w=1024, h=512)
    z = 100 * (ps[2, 1] < 256) - 50
    line2 = pano_connect_points(ps[2, :], ps[3, :], z, w=1024, h=512)
    return line1, line2
...
if __name__ == '__main__':
    a = getPanoRectangle([333, 194], [410, 397], [333, 397])


    pano_connect_points([876, 112], [217, 179], z=50)

    a = np.array([[20, 40], [200, 140], [800, 210]])
    cxy = np_coor2xy(a, z = 50)
    cuv = np_xy2coor(cxy, z = 50)
    a = 123